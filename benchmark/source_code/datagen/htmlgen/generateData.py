...
def genDataThread( mach, myID, *args ):
   ...
   os.system("ssh %s mkdir %s > /dev/null" %(mach, remoteDir))
   ...
   os.system("ssh %s mkdir %s" % (mach, params['Output']))
...
machs = range(int(split[0]),int(split[1])+1)
...
#   print "Start running Thread#", str(i)
   os.system('sleep 5')
   machineName = params['NamePrefix']
   if ( i < 10 ):
      machineName = machineName + "0"+str(i)
   else:
      machineName = machineName + str(i)
